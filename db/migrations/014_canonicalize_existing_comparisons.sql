WITH labels(section_order, section_title, label_order, label) AS (
  VALUES
    ...
    (4, 'Coding', 3, 'DeepSWE v1.1'),
    (4, 'Coding', 4, 'Terminal-Bench 2.1'),
    (4, 'Coding', 5, 'Terminal-Bench 4.0'),
    (4, 'Coding', 6, 'Terminal-Bench'),
    (4, 'Coding', 7, 'LiveCodeBench'),
    (4, 'Coding', 8, 'CursorBench'),
    ...
    (7, 'Agentic & computer use', 2, 'OSWorld 2.0'),
    (7, 'Agentic & computer use', 3, 'BrowseComp'),
    (7, 'Agentic & computer use', 4, 'GDPval-AA v2'),
    (7, 'Agentic & computer use', 5, 'MCP / tool-use benchmark')
),
resolved_pairs AS (
  SELECT
    c.id,
  ...
  LEFT JOIN LATERAL (
    SELECT m.slug, m.name, m.comparison_data
    FROM models AS m
    WHERE m.slug = c.metadata->>'modelA'
       OR lower(m.name) = lower(trim(split_part(c.title, ' vs ', 1)))
    ORDER BY CASE WHEN m.slug = c.metadata->>'modelA' THEN 0 ELSE 1 END
    LIMIT 1
  ) AS ma ON TRUE
  LEFT JOIN LATERAL (
    SELECT m.slug, m.name, m.comparison_data
    FROM models AS m
    WHERE m.slug = c.metadata->>'modelB'
       OR lower(m.name) = lower(trim(split_part(c.title, ' vs ', 2)))
    ORDER BY CASE WHEN m.slug = c.metadata->>'modelB' THEN 0 ELSE 1 END
    LIMIT 1
  ) AS mb ON TRUE
...
flat_existing_rows AS (
  SELECT
    ...
    row_ordinality
  FROM content_items AS c
  CROSS JOIN LATERAL jsonb_array_elements(COALESCE(c.blocks, '[]'::jsonb))
    WITH ORDINALITY AS block_value(block, block_ordinality)
  CROSS JOIN LATERAL jsonb_array_elements(
    CASE
      WHEN jsonb_typeof(block_value.block->'rows') = 'array' THEN block_value.block->'rows'
      ELSE '[]'::jsonb
    END
...
existing_rows AS (
  SELECT
    id,
    jsonb_object_agg(label, jsonb_build_array(value_a, value_b)) AS values_by_label
...
section_rows AS (
  ...
  WHERE p.model_a_slug IS NOT NULL
    AND p.model_b_slug IS NOT NULL
  GROUP BY p.id, l.section_order, l.section_title
),
sections AS (
  SELECT
    p.id,
    jsonb_agg(
      jsonb_build_object(
        'id', 'spec-canonical-' || sr.section_order,
        'type', 'spec_table',
...
canonical AS (
  SELECT
    ...
    s.blocks || jsonb_build_array(
      jsonb_build_object(
        'id', 'markdown-canonical-bottom-line',
        'type', 'markdown',
        'content', COALESCE(
          (
            SELECT old_block.block->>'content'
            FROM jsonb_array_elements(COALESCE(p.original_blocks, '[]'::jsonb))
              WITH ORDINALITY AS old_block(block, ordinality)
