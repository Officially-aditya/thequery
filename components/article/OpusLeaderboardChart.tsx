...
const data = [
  { model: "Claude Opus 5", lab: "Anthropic", score: 61 },
  ...
  { model: "Grok 4.5", lab: "xAI", score: 54 },
];
...
export default function OpusLeaderboardChart() {
  return (
    <figure
      ...
      aria-labelledby="opus-leaderboard-title"
    >
      <div className="font-sans text-[11px] uppercase tracking-[0.18em]" style={{ color: COLORS.muted }}>
        TheQuery data
      </div>
      <div
        id="opus-leaderboard-title"
        className="mt-2 font-serif text-2xl font-semibold sm:text-3xl"
        style={{ color: COLORS.text }}
      >
        Artificial Analysis Intelligence Index
      </div>
      <div className="mb-7 mt-3 max-w-2xl font-sans text-sm leading-relaxed" style={{ color: COLORS.muted }}>
        Opus 5 narrowly leads the field it launched into. The reaction did not match the result.
      </div>
      ...
      >
            ...
            >
              ...
              <Bar dataKey="score" radius={[0, 4, 4, 0]} barSize={26}>
                {data.map((entry) => (
                  <Cell
                    key={entry.model}
                    fill={entry.model === "Claude Opus 5" ? COLORS.neutral : COLORS.quiet}
                  />
                ))}
                <LabelList
                  dataKey="score"
        ...
        <div className="mt-3 font-sans text-[11px] leading-relaxed" style={{ color: COLORS.muted }}>
          Source: Artificial Analysis Intelligence Index, July 24–25, 2026 snapshot.
        </div>
      </div>
