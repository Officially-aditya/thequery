...
import { getAllIssues, getIssueBySlug } from "@/lib/digest";
...
export default async function DigestIssuePage({ params }: Props) {
  const { slug } = await params;
  const issue = getIssueBySlug(slug);
  if (!issue) notFound();
  ...
  return (
    <div className="max-w-[720px] mx-auto px-4 py-12">
      <Link href="/digest" className="text-sm text-text-muted hover:text-text-secondary transition-colors mb-6 inline-block">
        &larr; All Issues
      </Link>
